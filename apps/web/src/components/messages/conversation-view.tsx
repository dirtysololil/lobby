...
export function ConversationView({ conversationId, viewerId }: ConversationViewProps) {
  ...
  const loadConversation = useCallback(async () => {
    try {
      ...
      await apiClientFetch(`/v1/direct-messages/${conversationId}/read`, { method: "POST" });
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : "Не удалось загрузить диалог");
    }
  }, [conversationId]);
  ...
  async function sendMessage(content: string) {
    await apiClientFetch(`/v1/direct-messages/${conversationId}/messages`, { method: "POST", body: JSON.stringify({ content }) });
  ...
  async function saveSettings(payload: { notificationSetting: "ALL" | "MENTIONS_ONLY" | "MUTED" | "OFF"; retentionMode: "OFF" | "H24" | "D7" | "D30" | "CUSTOM"; customHours: number | null; }) {
    const response = await apiClientFetch(`/v1/direct-messages/${conversationId}/settings`, { method: "PATCH", body: JSON.stringify(payload) });
