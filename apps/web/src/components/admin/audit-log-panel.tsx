...
export function AuditLogPanel({ response }: AuditLogPanelProps) {
  return (
    <section className="social-shell rounded-[28px] p-6">
      <p className="font-mono text-xs uppercase tracking-[0.2em] text-[var(--text-muted)]">Журнал действий</p>
      <div className="mt-4 grid gap-3">
        {response.items.length === 0 ? <EmptyState title="Записей не найдено" description="Смягчите фильтры по действию или сущности." /> : response.items.map((item) => (
          <div key={item.id} className="rounded-2xl border border-[var(--border)] bg-[#0b1322]/70 p-4">
            ...
            <p className="mt-2 text-sm text-[var(--text-dim)]">{item.actor ? `${item.actor.profile.displayName} (@${item.actor.username})` : "Система"} · {new Date(item.createdAt).toLocaleString()}</p>
            <p className="mt-1 break-all text-xs text-[var(--text-muted)]">entityId: {item.entityId ?? "n/a"} · ip: {item.ipAddress ?? "n/a"}</p>
