  ...
  return (
    <div className="grid gap-5">
      <section className="social-shell rounded-[28px] p-6">
        <p className="font-mono text-xs uppercase tracking-[0.2em] text-[var(--text-muted)]">Модерация пользователей</p>
      ...
      <section className="social-shell rounded-[28px] p-6">
        <p className="font-mono text-xs uppercase tracking-[0.2em] text-[var(--text-muted)]">Пользователи</p>
        <div className="mt-4 grid gap-3">
          {response.items.length === 0 ? <EmptyState title="Ничего не найдено" description="Измените фильтры или расширьте поисковый запрос." /> : response.items.map((item) => (
            <div key={item.user.id} className="grid gap-4 rounded-2xl border border-[var(--border)] bg-[#0b1322]/70 p-4 xl:grid-cols-[minmax(0,1fr)_210px]">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-2 text-sm text-[var(--text-dim)]">Сессии: {item.activeSessionCount} · Хабы: {item.hubMembershipCount} · Последняя активность: {item.lastSeenAt ? new Date(item.lastSeenAt).toLocaleString() : "никогда"}</p>
