...
function TrackTile({ item }: { item: TrackView }) {
  ...
  useEffect(() => {
    ...
    const element =
      item.kind === "video" ? document.createElement("video") : document.createElement("audio");

    element.autoplay = true;
    element.setAttribute("playsinline", "true");

    if (element instanceof HTMLVideoElement) {
      element.className = "h-full w-full rounded-[24px] object-cover";
  ...
  return (
    <div className="rounded-[24px] border border-white/10 bg-slate-950/45 p-3">
      <div className="mb-3 flex items-center justify-between gap-3">
        ...
        <span className="rounded-full border border-white/10 px-3 py-1 text-[11px] uppercase tracking-[0.18em] text-sky-200/70">
          {item.kind}
        </span>
      </div>
      <div
        ref={containerRef}
        className="relative flex min-h-[180px] items-center justify-center overflow-hidden rounded-[20px] bg-slate-900/80"
      >
        {item.kind === "audio" ? <span className="text-sm text-slate-400">Audio stream active</span> : null}
      </div>
    </div>
  );
}
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!isCancelled) {
          setStatus("error");
          setErrorMessage(error instanceof Error ? error.message : "Unable to connect to LiveKit room");
        }
      }
    })();
    ...
    <Card>
      ...
      <CardContent className="space-y-5">
        <div className="flex flex-wrap gap-2 text-xs">
          <span className="rounded-full border border-white/10 px-3 py-1 text-sky-200/70">{mode}</span>
          <span className="rounded-full border border-white/10 px-3 py-1 text-slate-300">{status}</span>
          {!connection.canPublishMedia ? (
            <span className="rounded-full border border-amber-300/20 px-3 py-1 text-amber-100/80">listen only</span>
          ) : null}
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          <Button variant="secondary" onClick={() => void toggleMicrophone()} disabled={!connection.canPublishMedia}>
            {microphoneEnabled ? <Mic className="mr-2 h-4 w-4" /> : <MicOff className="mr-2 h-4 w-4" />}
            {microphoneEnabled ? "Mute" : "Unmute"}
          </Button>
          <Button variant="secondary" onClick={() => void toggleCamera()} disabled={!connection.canPublishMedia}>
            {cameraEnabled ? <Video className="mr-2 h-4 w-4" /> : <VideoOff className="mr-2 h-4 w-4" />}
            {cameraEnabled ? "Camera off" : "Camera on"}
          </Button>
          <Button variant="secondary" onClick={() => void toggleScreenShare()} disabled={!connection.canPublishMedia}>
            {screenShareEnabled ? (
          ...
          <Button onClick={() => void leaveRoom()} disabled={isLeaving}>
        ...
        {tracks.length === 0 ? (
          <div className="rounded-[24px] border border-white/10 bg-slate-950/35 p-5 text-sm text-slate-400">
            Waiting for published tracks...
          </div>
        ) : (
